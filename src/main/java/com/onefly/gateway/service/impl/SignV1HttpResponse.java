 ...
 */
@Slf4j
public class SignV1HttpResponse extends ServerHttpResponseDecorator {
        ...
        Mono<Result.Entity> modifiedBody = extractBody(clientResponse, Result.Entity.class)
                .flatMap(originalBody -> {
                    try {
                        ...
                        String sign = sign(client.getClientPrivateKey(), originalBody);
                        response.getHeaders().add(HttpHeader.SIGNATURE, sign);
                        return Mono.just(originalBody);
                    } catch (Exception e) {
                        log.error("响应结果处理失败: {}", e.getMessage(), e);
                        return Mono.error(e);
                    }
                });
...
//        }
        try {
            ...
            log.debug("签名数据[{}],响应数据签名:[{}]", bodyStr, sign);
            return sign;
        } catch (Exception e) {
