package com.onefly.gateway.filter.transform;
...
import com.onefly.gateway.utils.crypto.SignDataUtils;
...
public class TransformGatewayFilterFactory extends AbstractGatewayFilterFactory<TransformGatewayFilterFactory.Config> {
    ...
    private Mono<Void> versionLatest(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 目标地址
        String uriStr = Optional.ofNullable(headers.getFirst(HttpHeader.TARGET_HEADER_KEY)).orElseThrow(() -> new IllegalArgumentException("目标地址不能为空"));

        // 目标客户端ID
        String clientId = Optional.ofNullable(headers.getFirst(HttpHeader.CLIENT_ID)).orElseThrow(() -> new IllegalArgumentException("目标客户端ID不能为空"));
        ...
        newHeaders.remove(HttpHeader.TARGET_HEADER_KEY);
        newHeaders.remove(HttpHeader.CLIENT_ID);
            ...
            Mono<String> modifiedBody = serverRequest.bodyToMono(String.class)
                    .flatMap(body -> {
                        try {
                            ...
                            log.error("请求参数处理失败: {}", e.getMessage());
                            return Mono.error(e);
                        }
                    });
        ...
        Mono<byte[]> modifiedBody = serverRequest.bodyToMono(String.class)
                .flatMap(body -> {
                    try {
                        ...
                        String sign = SignDataUtils.getSign(data, client.getClientPrivateKey());
