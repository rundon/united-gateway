...
public class SignDataUtils {
    ...
    public static byte[] strToSignText(byte[] strData) {
        try {
            Map<String, Object> tmp = objectMapper.readValue(strData, new TypeReference<LinkedHashMap<String, Object>>() {
            });
            return getSignText(tmp);
        } catch (IOException e) {
            log.error("读取签名数据失败: {}", e.getMessage(), e);
        }
        return new byte[0];
    }
    ...
    public static String getSign(Object data, String base64PrivateKey) {
        ...
        try {
            ...
            log.debug("签名数据: [{}]", new String(signData));
            log.debug("签名: [{}]", sign);
            return sign;
